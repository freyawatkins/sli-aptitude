...
## Plots
Plot the linguistic & cognitive predictors over time 
...
( R1 <- apt %>% 
  filter(!is.na(dspan_time)) %>% 
  ggplot(aes(x = session, y = dspan_time, group = session, fill = session)) +
  geom_flat_violin(position = position_nudge(x = .25, y = 0),
                   adjust = 2, trim = TRUE, alpha = 1) +
  geom_point(position = position_jitter(width = .15), 
                        size = 0.5) +
...
( RJ <- apt %>% 
  ...
  geom_flat_violin(position = position_nudge(x = .2, y = 0),
                   adjust = 2, trim = TRUE, alpha = 1) +
  geom_point(data=indiv, aes(x = session, y = mean, group = ppt, color = session),
             size = 1.5, shape = 19, position = pd) +
  scale_fill_manual(values = c("#bae4bc", "#7bccc4", "#43a2ca", "#0868ac")) +
...
( nback_mdl <- lm(nback_comb ~ session, 
                  data = apt) ) 
...
( corsi_mdl <- lm(corsi_corr ~ session, 
                  data = apt) ) 
...
## Kirklees Sentence Reading
```{r kirk-mdl}
# Does session predict Kirklees results?
...
## KBIT-2 Matrices
```{r kbit-mdl}
# Does session predict K-Bit Matrices results?
...
## Digit Span
```{r dspan-mdl}
# Does session predict Kirklees Reading results?
( dspan_mdl <- lm(dspan_corr ~ session, 
                  data = apt) ) 
