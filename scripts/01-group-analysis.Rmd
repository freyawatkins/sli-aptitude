...
apt$drop_out <- as.factor(apt$drop_out)

apt$drop_out <- plyr::revalue(apt$drop_out, c("0" = "continuing", 
                                             "1" = "withdrew"))
apt %>% 
  group_by(drop_out) %>% 
  count()
...
```{r bsl-grade-models}
apt$drop_out <- as.numeric(apt$drop_out)
apt$drop_out <- plyr::revalue(apt$drop_out, c("3" = "1", 
                                             "1" = "0",
                                             "2" =))

( drop_out_mdl <- mixed(drop_out ~ nback_comb + corsi_corr, apt, 
