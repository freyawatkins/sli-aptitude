...
# Group Analysis
Here we are going to analyse data from session one across three groups (continuing, rejected, withdrew)
...
```{r mr2d-sats}
apt$selected <- as.factor(apt$selected) %>% plyr::revalue(c("0" = "not selected", 
                                            "1" = "selected"))

# Raincloud plot
( R1 <- apt %>% 
  filter(!is.na(mr2d_sats)) %>% 
  ggplot(aes(x = selected, y = mr2d_sats, group = selected)) +
  geom_flat_violin(position = position_nudge(x = .25, y = 0),
                   adjust = 2, trim = TRUE, alpha = 0.3) +
  geom_point(position = position_jitter(width = .15), 
                        size = 0.5)  +
...
# Jitter plot would not work for a trade-off score with bars above and below y=0
```

# Models
Which initial assessments best predict being selected for the degree?
```{r selected-model}
# Save #selected' as binary numeric variable again for regression models
apt$selected <- as.numeric(apt$selected) %>% -1
