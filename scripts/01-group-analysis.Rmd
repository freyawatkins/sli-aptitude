...
apt <- mutate(apt, across(1:4, factor))

# convert remaining character variables to numeric
apt <- mutate(apt, across(7:9, as.numeric))
apt <- mutate(apt, across(25, as.numeric))
apt <- mutate(apt, across(30:33, as.numeric))
apt <- mutate(apt, across(36:40, as.numeric))
```
...
( continue_mdl <- glm(continue ~ nback_comb + 
                                 ...
                                 kbit_acc + 
                                 mr3d_acc, 
                                 family = binomial, selected) ) 

summary(continue_mdl)
```

+ smaller analysis with the extra assesments done at S1 for the grant cohort
