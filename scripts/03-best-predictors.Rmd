...
```{r}
( fig4 <- (fig4a | fig4b) )
ggsave("plots/Fig4.png", plot=fig4, width = 12, height = 5, units = "in")
```
...
kirk0_bsl1 <- lm(`grade_bsl_1st year` ~ `kirk_acc_pre-degree` +
                   self_rating + age_s1, data = apt_wide)
summary(kirk0_bsl1)
...
```{r}
( fig5 <- (fig5a | fig5b) )
ggsave("plots/Fig5.png", plot=fig5, width = 12, height = 5, units = "in")
```
...
( fig6a <- apt_wide %>%
  ...
  theme_minimal() + theme_details + 
  labs(title = "a) 2nd yr Visuospatial WM vs. 3rd yr Eng>BSL interp.",
...
```{r}
( fig6 <- (fig6a | fig6b) )
ggsave("plots/Fig6.png", plot=fig6, width = 12, height = 5, units = "in")
```
...
( fig7a <- apt_wide %>%
  ...
  coord_cartesian(xlim = c(-1.3, 1.3)) +
  theme_minimal() + theme_details + 
  labs(title = "a) 2nd yr 2D Mental Rotation vs. 3rd yr BSL>Eng interp.",
...
ggsave("plots/Fig7.png", plot=fig7, width = 12, height = 8, units = "in")
```
...
e2b3_b2e3 <- lm(`terp_b2e_3rd year` ~ `terp_e2b_3rd year` +
                   self_rating, data = apt_wide)
summary(e2b3_b2e3) # significantly predict each other
...
e2b3_gt2 <- lm(`terp_e2b_3rd year` ~ `grade_terp_2nd year` +
                   self_rating, data = apt_wide)
summary(e2b3_gt2)
...
b2e3_gt2 <- lm(`terp_b2e_3rd year` ~ `grade_terp_2nd year` +
                   self_rating, data = apt_wide)
summary(b2e3_gt2)
...
gb2_gt2 <- lm(`grade_bsl_2nd year` ~ `grade_terp_2nd year` +
                   self_rating, data = apt_wide)
summary(gb2_gt2) # significantly correlate
...
gb1_gt2 <- lm(`grade_terp_2nd year` ~ `grade_bsl_1st year` +
                   self_rating, data = apt_wide)
summary(gb1_gt2) # signifcantly correlate
...
cs0_gb1 <- lm(`grade_bsl_1st year` ~ `copy_sign_pre-degree` +
                   self_rating, data = apt_wide)
summary(cs0_gb1) # interestingly no
...
cs0_gb2 <- lm(`grade_bsl_2nd year` ~ `copy_sign_pre-degree` +
                   self_rating, data = apt_wide)
summary(cs0_gb2) # interestingly no
...
cs0_gt2 <- lm(`grade_terp_2nd year` ~ `copy_sign_pre-degree` +
                   self_rating, data = apt_wide)
summary(cs0_gt2) # interestingly yes (marginal)
...
cs0_srt3 <- lm(`bsl_srt_3rd year` ~ `copy_sign_pre-degree` +
                   self_rating, data = apt_wide)
summary(cs0_srt3) 

# do BSL grades correlate with 3rd year BSL-SRT?
apt_wide %>%
...
gb1_srt3 <- lm(`bsl_srt_3rd year` ~ `grade_bsl_1st year` +
                   self_rating, data = apt_wide)
summary(gb1_srt3) # significant predictor 
...
gb2_srt3 <- lm(`bsl_srt_3rd year` ~ `grade_bsl_2nd year` +
                   self_rating, data = apt_wide)
summary(gb2_srt3) # marginal predictor
